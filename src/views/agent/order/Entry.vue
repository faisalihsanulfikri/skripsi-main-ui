        ...
        <div class="uk-card uk-card-default uk-card-small">
            ...
            <div class="uk-margin-small">
              <div class="uk-grid-small" uk-grid>
                ...
                <div style="width:13%">
                  ...
                  <el-select
                    v-model="input.item.goods.unitId"
                    name="unit"
                    slot="append"
                    :class="{ 'uk-form-danger': errors.has('unit') }"
                    @change="onUnitChanged"
                  >
                    <el-option value disabled selected>Unit</el-option>
                    <el-option
                      v-for="(item, key) in options.unit"
                      :key="key"
                      :value="item.value"
                      :label="item.label"
                    ></el-option>
              ...
              <ul v-if="input.item.goodsList.length > 0" class="uk-list">
                <li v-for="(goods, index) in input.item.goodsList" :key="index">
                  <div class="uk-grid-small" uk-grid>
                    ...
                    >{{ goods.name }} ( {{ goods.quantity }} {{ goods.unit }} )</div>
            ...
            <div class="uk-margin-small">
              <label class="uk-form-label">
                ...
                <el-tooltip
                  class="item"
                  effect="dark"
                  content="Total harga barang dalam paket."
                  placement="top"
                >
                  <font-awesome-icon icon="info-circle"></font-awesome-icon>
                </el-tooltip>
          ...
          <div class="uk-card-footer">
            <div class="uk-text-right">
              <button
                v-if="parseInt(input.consolidate) === 1"
                class="uk-button uk-button-default"
                @click="multiCheck"
              >Add</button>
              <template v-else>
            ...
            <tbody>
              <tr v-for="(item, index) in input.items" :key="index">
                ...
                <td class="uk-text-right">
                  {{ item.price | currency('', 2, { thousandsSeparator: '.', decimalSeparator:
                  ',' }) }}
                </td>
                <td class="uk-text-center">{{ item.weight }}</td>
                <td class="uk-text-center">
                  <template
                    v-if="item.length > 0 && item.weight > 0 && item.height > 0"
                  >{{ `${item.length} x ${item.width} x ${item.height}` }}</template>
                </td>
                <td class="uk-text-center">
...
export default {
  ...
  methods: {
    ...
    onCurrencyChanged() {
      this.input.item.price =
        this.input.item.currencyPrice * this.input.item.currency;
    ...
    onConsolidateChanged() {
      this.$confirm(
        "The package and items you input will be lost, are you sure?",
        "warning",
        {
          type: "warning"
        }
        ...
        .catch(() => {
          this.input.consolidate =
            parseInt(this.input.consolidate) === 0 ? 1 : 0;
        });
    },
    onInsuranceChanged() {
    ...
    async onUserSelected(user) {
      ...
      try {
        ...
        this.options.address = res.data.map(item => {
          let $item = {
            value: item.id,
            label: `${item.address} - ${item.province} - ${item.city} - ${
              item.sub_district
            } `
          };
